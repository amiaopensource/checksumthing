...
from hashing import read_hash, modify_hash, write_hash, get_hash_length
...
def main():
    ...
    try:
        hash_length = get_hash_length(args.t)
    ...
    for filename in files:
        ...
        old_hash = read_hash(filename, hash_length=hash_length)

        # Create a new hash to replace the exiting
        new_hash = modify_hash(old_hash, args=args)

        # Overwrite the original file with the new has
        write_hash(new_hash, filename)
